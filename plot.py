...
include_number = 7 #Number of orgs to include
...
def get_plot():
    ...
    for i,(dates,dons,org) in enumerate(zip(days_by_org, donations_by_org, sorted_orgs)): #All organizations
        j = rows-i
        ...
        row=j, col=1)
        fig.update_xaxes(range=[0,4],row=j, col=1)
        ...
        fig.update_xaxes(range=[0,1],row=j, col=2)
        ...
        fig.add_trace(
            go.Scatter(
                x=[dates[0], dates[-1]], 
                y=[np.cumsum(dons)[0],np.cumsum(dons)[-1]],
                mode='markers',
        ...
        fig.update_xaxes(range=[min_date,max_date],row=j, col=3) #Use instead of share_xaxes because that hides traces for some reason
        
        # Histogram column
        fig.add_trace(
            go.Scatter(
                x=[0, 0, sum(dons), sum(dons), 0],
                y=[1,-1,-1,1,1], 
                mode='lines', 
                line=dict(color='black'), 
                fill='toself', 
        ...
        fig.add_trace(
            go.Scatter(
                x=[sum(dons)+0.02*max_total_donation] ,
                y=[0], 
                mode='text', 
                text=f'{np.cumsum(dons)[-1]/1e6:.3f}', 
                textposition='middle right',
                cliponaxis=False,
                ), row=j, col=4)
        fig.update_xaxes(range=[0,max_total_donation*1.25],row=j, col=4)
        fig.update_yaxes(range=[-1.1,1.1],row=j, col=4)
    ...
    fig.update_layout(
        ...
        #font=dict(family="Sigmar", size=15, color='black')
    )
    ...
    #[fig.update_xaxes(range=[0,1], row=j, col=2) for j in range(len(sorted_orgs))]
    fig.update_xaxes(
        showticklabels=True, 
        ticks='outside',
        dtick='M12',
